...
import com.example.vehicles.databinding.ActivitySignupBinding
...
class SignupActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.signupButton.setOnClickListener {
            ...
            val isUser = binding.signupIsUser.isChecked

            if (signupUsername.isNotEmpty() && signupPassword.isNotEmpty()) {
                signUpUser(signupUsername, signupPassword, isUser)
            } else {
    ...
    private fun signUpUser(email: String, password: String, isUser: Boolean) {
        firebaseAuth.createUserWithEmailAndPassword(email, password).addOnCompleteListener(this) { task ->
            if (task.isSuccessful) {
                val userId = firebaseAuth.currentUser?.uid
                val role = if (isUser) "user" else "admin"
